...
void test_loopback_send_recv_basic()
{
    ...
    uint64_t src_addr         = 1;
    uint64_t dst_addr         = 2;
    bool send_result          = transport1.send(send_data, 4, src_addr, dst_addr);
    ...
    size_t recv_len = transport2.recv(recv_buffer, 10, recv_src_addr, recv_dst_addr);
...
void test_loopback_available_transitions()
{
    ...
    transport1.send(data, 1, 1, 2);

    TEST_ASSERT_TRUE(transport2.available());

    uint8_t buffer[1];
    uint64_t src_addr, dst_addr;
    transport2.recv(buffer, 1, src_addr, dst_addr);
...
void test_loopback_send_fails_without_peer()
{
    ...
    bool result          = transport.send(data, 2, 1, 2);
...
void test_loopback_zero_length_send()
{
    ...
    bool result          = transport1.send(data, 0, 1, 2);
...
void test_loopback_empty_queue_recv()
{
    ...
    uint8_t buffer[10];
    uint64_t src_addr, dst_addr;
    size_t recv_len = transport.recv(buffer, 10, src_addr, dst_addr);
...
void test_loopback_cross_peer_propagation()
{
    ...
    transport1.send(data1, 2, 1, 2);
    transport2.send(data2, 2, 2, 1);
    ...
    size_t len1 = transport2.recv(buffer1, 2, src_addr1, dst_addr1);
    size_t len2 = transport1.recv(buffer2, 2, src_addr2, dst_addr2);
...
void test_loopback_partial_recv()
{
    ...
    transport1.send(data, 5, 10, 20);

    uint8_t buffer[2];
    uint64_t src_addr, dst_addr;
    size_t len1 = transport2.recv(buffer, 2, src_addr, dst_addr);
    ...
    size_t len2 = transport2.recv(buffer, 2, src_addr, dst_addr);
    ...
    size_t len3 = transport2.recv(buffer, 2, src_addr, dst_addr);
